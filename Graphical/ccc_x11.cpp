...
void GraphicWindow::open(Display* d, Window w)
{
   ...
   {   std::cerr << "Unable to initialize font: " << SYS_FONT_NAME << std::endl;
      exit(1);
    }
...
void GraphicWindow::statusline_prompt(string s)
{
   ...
   XDrawImageString(display, _ppm, xgc, 0, ascent, t, (int)strlen(t));
}
...
int GraphicWindow::put_string(const char instr[], int str_x, int curr_x)
{
   ...
   XSetForeground(display, xgc, BlackPixel(display, screen_num));
   XDrawImageString(display, win, xgc, str_x, ascent, instr, (int)strlen(instr));
...
int main(int argc, char** argv)
{
   ...
   char res[7] = {'c','h','i','g','c','x','\0'};
   class_hints->res_class = res;
